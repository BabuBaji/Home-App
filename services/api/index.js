...
// Internal (service-to-service): the Auth service owns identity/login but delegates
// user-PROFILE provisioning to here (bookings/wallet FK to users live in this DB).
const internalOk = (req) => !process.env.INTERNAL_KEY || req.headers['x-internal-key'] === process.env.INTERNAL_KEY
app.post('/api/internal/users/find-or-create', (req, res) => {
...
app.post('/api/internal/users/find-or-create-google', (req, res) => {
  if (!internalOk(req)) return res.status(403).json({ error: 'forbidden' })
