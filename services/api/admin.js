...
import { adminInsights } from './admin-metrics.js'
...
export function createAdminRouter(io) {
  ...
  // Rich analytics/insights — all computed live from the DB (Analytics & Reports screens).
  r.get('/insights', admin, (_q, res) => res.json(adminInsights()))

  // Actionable-alert count for the top-bar bell (open complaints + unresolved tickets).
  r.get('/alerts', admin, (_q, res) => {
    const complaints = db.prepare("SELECT COUNT(*) n FROM complaints WHERE status IN ('open','in_progress')").get().n
    const tickets = db.prepare("SELECT COUNT(*) n FROM tickets WHERE LOWER(COALESCE(status,'')) NOT IN ('resolved','closed')").get().n
    res.json({ count: complaints + tickets, complaints, tickets })
  })
  ...
  r.get('/activity', admin, async (req, res) => res.json(await listActivity({
  ...
  r.get('/activity/stats', admin, async (req, res) => res.json(await activityStats(Number(req.query.days) || 7)))
  r.get('/bookings/:id/timeline', admin, async (req, res) => res.json(await bookingTimeline(Number(req.params.id))))
