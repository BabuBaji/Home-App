      ...
      {tab === 'activity' && (!activity ? <Loading /> : (() => {
        ...
        <div className="grid" style={{ gridTemplateColumns: 'minmax(0, 2.6fr) minmax(240px, 1fr)', gap: 16, alignItems: 'start' }}>
          <div className="grid" style={{ gap: 16 }}>
            {/* filters card */}
            <Card>
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(240px, 1.7fr) repeat(3, minmax(130px, 1fr))', gap: 14, alignItems: 'end' }}>
                <div><label className="muted" style={{ fontSize: 11, display: 'block', marginBottom: 6, fontWeight: 600 }}>Date Range</label><div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><input type="date" className="input" style={{ flex: 1, minWidth: 0 }} value={aFrom} onChange={(e) => setAFrom(e.target.value)} /><span className="muted">–</span><input type="date" className="input" style={{ flex: 1, minWidth: 0 }} value={aTo} onChange={(e) => setATo(e.target.value)} /></div></div>
