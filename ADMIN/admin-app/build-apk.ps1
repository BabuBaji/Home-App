...
# 3) First run? add the Android platform
if (-not (Test-Path "$root\android")) { npx cap add android; if ($LASTEXITCODE) { throw 'cap add android failed' } }

# 4) Generate launcher icon + splash from assets/ (adaptive icon, all densities)
npx @capacitor/assets generate --android
if ($LASTEXITCODE) { throw 'icon/splash generation failed' }
...
# 5) Assemble APK (via cmd so Gradle's stderr warnings don't abort the script)
cmd /c "cd /d `"$root\android`" && .\gradlew.bat assembleDebug --no-daemon 2>&1"
if ($LASTEXITCODE) { throw 'gradle build failed' }

# 6) Copy to repo root
$apk = "$root\android\app\build\outputs\apk\debug\app-debug.apk"
$dest = "$root\..\..\HomeHelp-Admin-debug.apk"
Copy-Item $apk $dest -Force
$mb = [math]::Round((Get-Item $dest).Length / 1MB, 2)
Write-Host "`nAPK ready: $dest  ($mb MB)" -ForegroundColor Green
Write-Host "App points to http://$ip`:4000 - make sure the backend is running (npm run server in customer-app)." -ForegroundColor Yellow
